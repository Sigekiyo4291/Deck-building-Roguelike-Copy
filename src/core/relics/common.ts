...
export const CommonRelics = {
    ...
    OMAMORI: new class extends Relic {
        ...
        onObtain(owner: any) { owner.relicCounters['omamori'] = 2; }
        // 呪い無効化の判定はカード追加時にフック追加済
        isUsedUp(owner: any) { return owner.relicCounters['omamori'] === 0; }
    },
    MEAL_TICKET: new class extends Relic {
        constructor() { super('meal_ticket', 'お食事券', 'ショップに来店するたび、HPを15回復。', 'common'); }
        onRoomEnter(owner: any, roomType: any) { if (roomType === RoomType.SHOP) owner.heal(15); }
    },
    ...
    MAW_BANK: new class extends Relic {
        ...
        onGoldSpend(owner: any, amount: any) { owner.relicCounters['maw_bank_broken'] = true; }
        isUsedUp(owner: any) { return !!owner.relicCounters['maw_bank_broken']; }
    },
    RED_SKULL: new class extends Relic {
        constructor() { super('red_skull', 'レッドスカル', 'HPが50％以下になると、筋力3を得る。', 'common'); }
        checkEffect(owner: any) {
        ...
        onBattleStart(owner: any, engine: any) { this.checkEffect(owner); }
        onTakeDamage(owner: any, engine: any, amount: any) { this.checkEffect(owner); }
        onHPRecovery(owner: any, engine: any, amount: any) { this.checkEffect(owner); }
    },
    ANCIENT_TEA_SET: new class extends Relic {
        constructor() { super('ancient_tea_set', '古代のティーセット', '休憩場所を通過した次の戦闘において、●●を得た状態でスタートする。', 'common'); }
        onRoomEnter(owner: any, roomType: any) { if (roomType === RoomType.REST) owner.relicCounters['ancient_tea_set'] = 1; }
        onPlayerTurnStart(owner: IEntity, engine: IBattleEngine) {
    ...
    ART_OF_WAR: new class extends Relic {
        ...
        onCardPlay(owner: any, engine: any, card: any) { if (card.type === 'attack') owner.relicCounters['art_of_war_played_attack'] = true; }
        onTurnEnd(owner: any, engine: any) { if (!owner.relicCounters['art_of_war_played_attack']) owner.relicCounters['art_of_war_trigger'] = true; }
    },
    ...
    WAR_PAINT: new class extends Relic {
        ...
        onObtain(owner: any) {
            const skills = owner.masterDeck.filter((c: any) => c.type === 'skill' && !c.isUpgraded && c.upgradeData);
    ...
    TOY_ORNITHOPTER: new class extends Relic {
        ...
        onPotionUse(owner: any, potion: any) { owner.heal(5); }
    },
    CENTENNIAL_PUZZLE: new class extends Relic {
        constructor() { super('centennial_puzzle', '百年パズル', '戦闘中初めてHPを失うと、カードを3枚引く。', 'common'); }
        onBattleStart(owner: any, engine: any) { owner.relicCounters['centennial_puzzle'] = false; }
    ...
    WHETSTONE: new class extends Relic {
        ...
        onObtain(owner: any) {
            const attacks = owner.masterDeck.filter((c: any) => c.type === 'attack' && !c.isUpgraded && c.upgradeData);
    ...
    AKABEKO: new class extends Relic {
        ...
        onBattleStart(owner: any, engine: any) { owner.relicCounters['akabeko'] = true; owner.addStatus('vigor', 8); } // vigor(活力)で表現
        modifyDamageDealt(owner: any, target: any, damage: any, card?: any) {
    ...
    CERAMIC_FISH: new class extends Relic {
        ...
        onCardAdd(owner: any, card: any) { owner.gold += 9; }
    },
