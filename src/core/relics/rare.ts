...
export const RareRelics = {
    ...
    DU_VU_DOLL: new class extends Relic {
        ...
        onBattleStart(owner: IEntity, engine: IBattleEngine) {
            const curses = (owner as IPlayer).masterDeck.filter((c: any) => (c as any).type === 'curse').length;
            if (curses > 0) {
    ...
    CHARONS_ASHES: new class extends Relic {
        ...
        onCardExhaust(owner: IEntity, engine: IBattleEngine, card: any) {
            if (!engine) return;
    ...
    MAGIC_FLOWER: new class extends Relic {
        ...
        modifyHealAmount(owner: IEntity, amount: number) {
    ...
    CAPTAINS_WHEEL: new class extends Relic {
        ...
        isUsedUp(owner: any) { return owner.relicCounters['captains_wheel'] === 3; }
    },
    ...
    DEAD_BRANCH: new class extends Relic {
        ...
        onCardExhaust(owner: IEntity, engine: IBattleEngine, card: any) {
            if (!engine) return;
            const allCards = Object.values(CardLibrary).filter((c: any) => (c as any).type !== 'basic' && (c as any).type !== 'curse' && (c as any).type !== 'status');
            const randomCard = (allCards[Math.floor(Math.random() * allCards.length)] as any).clone();
    ...
    FOSSILIZED_HELIX: new class extends Relic {
        ...
        isUsedUp(owner: any) { return owner.relicCounters['fossilized_helix'] === 0; }
    },
    CHAMPION_BELT: new class extends Relic {
        constructor() { super('champion_belt', 'チャンピオンベルト', '敵に「脆弱」を付与するたび、対象に「脱力」1を付与する。', 'rare'); }
        onApplyStatus(owner: any, target: any, type: any, value: any, engine: any) {
    ...
    LIZARD_TAIL: new class extends Relic {
        ...
        isUsedUp(owner: any) { return owner.relicCounters['lizard_tail'] === 0; }
    },
