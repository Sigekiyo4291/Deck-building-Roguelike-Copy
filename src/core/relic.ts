...
export const RelicLibrary = {
    ...
    MARK_OF_PAIN: new class extends Relic {
        ...
        onBattleStart(owner, engine) {
            ...
            const wound1 = CardLibrary.WOUND ? CardLibrary.WOUND.clone() : null;
            const wound2 = CardLibrary.WOUND ? CardLibrary.WOUND.clone() : null;
            if (wound1) engine.addCardToDrawPile(wound1);
            if (wound2) engine.addCardToDrawPile(wound2);
            console.log('苦痛の印発動！ 山札に負傷を2枚追加しました。');
        }
    },

    // Phase 2: Obtaining / Deck Manipulation
    ...
    BLACK_BLOOD: new class extends Relic {
        ...
        onObtain(owner) {
            // 燃焼の血液を削除
            owner.relics = owner.relics.filter(r => r.id !== 'burning_blood');
        }
        onBattleWin(owner) {
            owner.heal(12);
    ...
    // Phase 3: Special Combat Mechanics
    SNECKO_EYE: new class extends Relic {
        ...
        onBattleStart(owner) { owner.addStatus('confusion', 99); }
        onPlayerTurnStart(owner, engine) { engine.drawCards(2); }
    },
    RUNIC_CUBE: new class extends Relic {
        constructor() { super('runic_cube', 'ルーニック・キューブ', 'ダメージを受けるたびにカードを1枚引く。', 'boss'); }
        onTakeDamage(owner, amount, engine) {
